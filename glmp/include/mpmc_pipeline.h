...
#define PPL_DEAFULT_DISPATCH_SCHED 3
    thread_local auto sched_v = co::all_schedulers();
...
//            else std::thread(loop).detach();
    ...
    template<typename F>
    _pipe_base* operator|(_pipe_base* left, F f){
    ...
    Segment* parallel(_pipe_base* head, int n_parallel=2, int n_concurrent=64){
